...
- **Host (`lazarus-hostd`):** runs with user privileges; brokers everything.
- **Runner (`lazarus-runnerd`):** supervises agent processes.
...
```text
User <---- authenticated loopback IPC ----> Host
Host  ---- process control ----------->  Runner -> agent processes
...
| #   | Threat                                                                            | Mitigations                                                                                                                                                |
| --- | --------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| T1  | Another local process connects to Host IPC and drives agents/secrets              | Loopback-only listeners; named pipe/domain socket ACLs; high-entropy per-install token; strict Origin validation for HTTP/WS; no pre-auth workspace paths  |
