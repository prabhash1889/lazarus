# 0001 - Foundational architecture: local-first, host/runner split, provider-neutral core
...
2. **Host/runner boundary.** `lazarus-hostd` owns durable state and orchestration; `lazarus-runnerd` is a deliberately small process supervisor owning PTYs and agent child processes. Host restarts must not needlessly terminate compatible supervised processes; runners reconcile against Host state after restart.
...
4. **Protocol boundaries are real.** Desktop and CLI communicate with the Host through a versioned protocol (handshake, capability negotiation, unary + streaming), not in-process state sharing. Compatibility rules: `docs/protocol/compatibility.md`.
...
- **Single process combining Host + runner supervision:** rejected - Host restarts/updates would kill live agent processes; the split enables drain/reconcile lifecycle semantics.
