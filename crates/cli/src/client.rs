//! Shared authenticated-transport helpers for every CLI command that talks
//! to the Host: loopback validation, local-token resolution, contract
//! headers, deadline-matched timeouts, and manifest verification.
...
pub(crate) fn validate_loopback_addr(addr: &str) -> Result<()> {
...
pub(crate) fn resolve_token(raw: Option<&str>) -> Result<String> {
    match raw.map(str::trim) {
        None => {
            bail!("{LOCAL_TOKEN_ENV} is not set and no per-install token file exists")
        }
...
/// The token from the raw environment when it is present, else the
/// per-install token file `host start` provisions under the data root.
pub(crate) fn local_token(paths: &lazarus_hostd::runtime::DataPaths) -> Result<String> {
    match std::env::var(LOCAL_TOKEN_ENV) {
        Ok(raw) => resolve_token(Some(&raw)),
        Err(_) => discovery::read_token(paths)?.ok_or_else(|| {
            anyhow!("no local token available; run `lazarus host start` or set {LOCAL_TOKEN_ENV}")
        }),
    }
}
...
pub(crate) fn contract_headers(token: &str) -> Result<Vec<(HeaderName, HeaderValue)>> {
...
pub(crate) fn client_timeout() -> Duration {
...
/// negotiation outcome.
pub(crate) fn verify_response_manifest(raw: Option<&str>) -> Result<NegotiatedManifest> {
...
/// secrets.
pub(crate) fn describe_host_error(status: u16, body: &str) -> String {
...
pub(crate) fn format_method_resolution(name: &str, resolution: &Resolution) -> String {
    match resolution {
...
/// Performs one authenticated contract GET and verifies the response
...
pub(crate) async fn fetch_json(
    ...
    let response = request.send().await.with_context(|| {
        format!("cannot reach host at {addr}; start it with `lazarus host start`")
    })?;
...
/// Issues one authenticated POST to `path` on the Host, discarding the body.
pub(crate) async fn post(
    ...
    let url = format!("{}{}", addr.trim_end_matches('/'), path);
    let mut request = client.post(&url).timeout(client_timeout());
    for (name, value) in contract_headers(token)? {
        request = request.header(name, value);
    }
...
mod tests {
    ...
    fn rejects_missing_or_empty_local_token_before_any_network_use() {
        assert_eq!(
            ...
            format!("{LOCAL_TOKEN_ENV} is not set and no per-install token file exists")
        );
        assert_eq!(
